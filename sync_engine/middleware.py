...
class AutoSyncMiddleware:
    ...
    def __call__(self, request):
        ...
        # 1. RECONNECTION TRIGGER: Offline -> Online (or first time online)
        if is_online and not self._last_online_status:
        ...
        # 2. PERIODIC FULL SYNC: If online and no full sync in the last 10 minutes
        ...
        # 3. IMMEDIATE PUSH: After any successful write request when online.
        # We push on every mutating request (POST/PUT/PATCH/DELETE) whose
        # response is a success (< 400), so local edits reach the cloud as
        # soon as the user saves them.
        if (
